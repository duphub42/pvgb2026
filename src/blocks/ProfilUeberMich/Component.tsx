...
import { profilUeberMichEinleitungDefault, profilWerteDefaults } from '@/blocks/ProfilBlocks/defaults'
...
export const ProfilUeberMichBlock: React.FC<Props> = ({
  ...
  const fromCms = (werte ?? []).filter(
    (w): w is NonNullable<(typeof werte)[number]> =>
  ...
  return (
    <section className={cn('container border-b border-border/60 py-16 md:py-20')}>
      <h2 className="mb-8 text-2xl font-bold tracking-tight text-foreground md:text-3xl">{title}</h2>

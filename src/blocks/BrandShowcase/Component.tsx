...
import type { BrandShowcaseBlock as BrandShowcaseBlockData, Media as MediaType } from '@/payload-types'
...
export const BrandShowcaseBlock: React.FC<BrandShowcaseProps> = ({
  ...
  const typeRows = (typography ?? []).filter((row) => Boolean(row?.role?.trim() && row?.family?.trim()))
  const principleRows = (principles ?? []).filter(
    (row) => Boolean(row?.title?.trim() && row?.description?.trim()),
  )
  const usageRows = (usageExamples ?? []).filter((row) => typeof row?.image === 'object' && row.image)
      ...
      <div className="container rounded-3xl border border-border/70 bg-gradient-to-br from-zinc-50 via-white to-stone-50 px-6 py-8 md:px-10 md:py-12">
        ...
        <div className="mt-8 grid gap-5 xl:grid-cols-12">
          <div className="space-y-5 xl:col-span-5">
            {(typeof logo === 'object' && logo) || (typeof wordmark === 'object' && wordmark) ? (
              <div className="rounded-2xl border border-border/70 bg-white p-5">
                <p className="mb-4 text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Logosystem</p>
                <div className="grid gap-4 sm:grid-cols-2">
                  {typeof logo === 'object' && logo ? (
                    <div className="rounded-xl border border-border/60 bg-zinc-50 p-4">
            ...
            {brandStory ? (
              <div className="rounded-2xl border border-border/70 bg-white p-5">
                <p className="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Narrative</p>
                <p className="text-sm leading-relaxed text-muted-foreground md:text-base">{brandStory}</p>
              </div>
            ) : null}

            {principleRows.length > 0 ? (
              <div className="rounded-2xl border border-border/70 bg-white p-5">
                <p className="mb-4 text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Design-Prinzipien</p>
                <ul className="space-y-3">
                  {principleRows.map((principle, index) => (
                    <li
                      key={typeof principle.id === 'string' && principle.id ? principle.id : `principle-${index}`}
                      className="rounded-xl border border-border/60 bg-zinc-50 p-3"
                    >
          ...
          <div className="space-y-5 xl:col-span-7">
            {colorRows.length > 0 ? (
              <div className="rounded-2xl border border-border/70 bg-white p-5">
                <p className="mb-4 text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Farbpalette</p>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {colorRows.map((row, index) => {
                    ...
                    return (
                      <article key={key} className="overflow-hidden rounded-xl border border-border/70">
                        <div className="h-16" style={{ backgroundColor: swatch }} aria-hidden />
                        <div className="space-y-1 bg-white px-3 py-2.5">
                          <p className="text-sm font-semibold">{row.name}</p>
                          <p className="text-xs font-mono text-muted-foreground">{swatch.toUpperCase()}</p>
                          {row.usage ? (
                            <p className="text-xs leading-relaxed text-muted-foreground">{row.usage}</p>
                          ) : null}
                        </div>
                      </article>
                    )
            ...
            {typeRows.length > 0 ? (
              <div className="rounded-2xl border border-border/70 bg-white p-5">
                <p className="mb-4 text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Typografie</p>
                <div className="grid gap-3 lg:grid-cols-2">
                  {typeRows.map((row, index) => (
                    ...
                    >
                      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{row.role}</p>
        ...
        {usageRows.length > 0 ? (
          <div className="mt-8">
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Anwendungsbeispiele</p>

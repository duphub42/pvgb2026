/**
 * Fuegt Verband Digitale Innovation als zentralen Portfolio-Case hinzu.
 *
 * Ausfuehren:
 *   npx tsx src/scripts/upsert-verband-digitale-innovation-portfolio-case.ts
 *   npx tsx src/scripts/upsert-verband-digitale-innovation-portfolio-case.ts --dry-run
 */
...
const CASE_TITLE = 'Verband Digitale Innovation'
const MOCKUP_PATH = '/Users/horus/Downloads/verband-digitale-innovation-mockup.png'
...
async function getOrCreateMedia(dryRun: boolean): Promise<number | undefined> {
  ...
  const existing = await payload.find({
    ...
    where: {
      or: [
        { filename: { equals: 'verband-digitale-innovation-mockup.png' } },
        { alt: { equals: 'Verband Digitale Innovation Website-Mockup' } },
      ],
    },
    limit: 1,
    depth: 0,
  ...
  if (!fs.existsSync(MOCKUP_PATH)) {
    console.warn(`Mockup nicht gefunden: ${MOCKUP_PATH}`)
    return undefined
  }

  if (dryRun) {
    console.log(`[dry-run] Wuerde Media-Upload anlegen: ${MOCKUP_PATH}`)
    return undefined
  }
  ...
  const media = await payload.create({
    collection: 'media',
    data: {
      alt: 'Verband Digitale Innovation Website-Mockup',
    },
    filePath: MOCKUP_PATH,
    overrideAccess: true,
  })
...
function buildCase(coverImageId?: number, existing?: PortfolioCase): PortfolioCase {
  return stripNestedIds({
    ...(existing ?? {}),
    discipline: 'mixed',
    title: CASE_TITLE,
    client: 'Verband Digitale Innovation',
    industry: 'Verein fuer digitale Innovation / Expertennetzwerk',
    year: 2023,
    categories: ['komplettDesign', 'content', 'uxUi'],
    ...RECENT_PORTFOLIO_CASE_COPY[CASE_TITLE],
    metrics: [
      { value: '2023', label: 'Projektjahr' },
      { value: '360°', label: 'Verbandsauftritt' },
    ],
    tags: [{ label: 'Webdesign' }, { label: 'Expertennetzwerk' }, { label: 'Digitale Innovation' }],
    website: {
...
async function main() {
  ...
  const payload = await getPayload({ config })
  const coverImageId = await getOrCreateMedia(dryRun)
  ...
  const existingIndex = cases.findIndex(
    (entry) =>
      String(entry.title ?? '') === CASE_TITLE && String(entry.discipline ?? 'mixed') === 'mixed',
  )

  const nextCases = [...cases]
  const nextCase = buildCase(
    coverImageId,
  ...
  const nextCentralBlock = buildCentralPortfolioCaseBlock({
    ...centralBlock,
    intro:
      'Eine Auswahl realer Kundenprojekte aus Webdesign, Marketing und Branding - inklusive SEO-, Lead-, Verbands- und E-Commerce-Referenzen mit messbaren Ergebnissen.',
    cases: updatedCases,
  ...
  console.log(`Ziel: /${CENTRAL_PORTFOLIO_PAGE_SLUG} -> ${CASE_TITLE}`)
  ...
  console.log('\nFertig. Verband Digitale Innovation ist im zentralen Portfolio-Slider gepflegt.')
}

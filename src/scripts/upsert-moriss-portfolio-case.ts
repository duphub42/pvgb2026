/**
 * Fuegt Moriss Obstplantagen als zentralen Portfolio-Case hinzu.
 *
 * Achtung: Das Script schreibt in die konfigurierte Payload-Datenbank.
 * Erst mit --dry-run pruefen:
 *   npx tsx src/scripts/upsert-moriss-portfolio-case.ts --dry-run
 *   npx tsx src/scripts/upsert-moriss-portfolio-case.ts
 */
...
const CASE_TITLE = 'Moriss Obstplantagen'
const MOCKUP_PATH =
  '/Users/horus/Desktop/pvgb2026/pvgb2026/public/media/moriss-obsthof-showcase.png'
const MEDIA_ALT = 'Moriss Obstplantagen Website-Showcase'
...
async function getOrCreateMedia(dryRun: boolean): Promise<number | undefined> {
  ...
  const existing = await payload.find({
    ...
    where: {
      or: [
        { filename: { equals: 'moriss-obsthof-showcase.png' } },
        { alt: { equals: MEDIA_ALT } },
      ],
    },
    limit: 1,
    depth: 0,
...
async function main() {
  ...
  const nextCentralBlock = buildCentralPortfolioCaseBlock({
    ...centralBlock,
    intro:
      'Eine Auswahl realer Kundenprojekte aus Webdesign, Marketing und Branding - inklusive spezialisierten Branchen-Websites, SEO-, Lead- und E-Commerce-Referenzen.',
    cases: updatedCases,
  ...
  console.log('\nFertig. Moriss Obstplantagen ist im zentralen Portfolio-Slider gepflegt.')
}

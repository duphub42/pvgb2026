/**
 * Fuegt KIPP Dental als zentralen Portfolio-Case hinzu.
 *
 * Ausfuehren:
 *   npx tsx src/scripts/upsert-kipp-dental-portfolio-case.ts
 *   npx tsx src/scripts/upsert-kipp-dental-portfolio-case.ts --dry-run
 */
...
const CASE_TITLE = 'KIPP Dental'
const MOCKUP_PATH = '/Users/horus/Downloads/Kipp-dental-mockup.png'
...
async function getOrCreateMedia(dryRun: boolean): Promise<number | undefined> {
  ...
  const existing = await payload.find({
    ...
    where: {
      or: [
        { filename: { equals: 'Kipp-dental-mockup.png' } },
        { alt: { equals: 'KIPP Dental Website-Mockup fuer Dentallabor und Medizintechnik' } },
      ],
    },
    limit: 1,
    depth: 0,
  ...
  const media = await payload.create({
    collection: 'media',
    data: {
      alt: 'KIPP Dental Website-Mockup fuer Dentallabor und Medizintechnik',
    },
...
async function main() {
  ...
  const nextCentralBlock = buildCentralPortfolioCaseBlock({
    ...centralBlock,
    intro:
      'Eine Auswahl realer Kundenprojekte aus Webdesign, Marketing und Branding - inklusive SEO-, Lead-, Medizin- und E-Commerce-Referenzen mit messbaren Ergebnissen.',
    cases: updatedCases,
  ...
  console.log('\nFertig. KIPP Dental ist im zentralen Portfolio-Slider gepflegt.')
}

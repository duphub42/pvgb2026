/**
 * Fuegt Zahnarzt Kipp als zentralen Portfolio-Case hinzu.
 *
 * Ausfuehren:
 *   npx tsx src/scripts/upsert-zahnarzt-kipp-portfolio-case.ts
 *   npx tsx src/scripts/upsert-zahnarzt-kipp-portfolio-case.ts --dry-run
 */
...
const CASE_TITLE = 'Zahnarzt Kipp'
const MOCKUP_PATH = '/Users/horus/Downloads/Zahnarzt-Kipp-mockup.png'
...
async function getOrCreateMedia(dryRun: boolean): Promise<number | undefined> {
  ...
  const existing = await payload.find({
    ...
    where: {
      or: [
        { filename: { equals: 'Zahnarzt-Kipp-mockup.png' } },
        { alt: { equals: 'Zahnarzt Kipp Website-Mockup' } },
      ],
    },
    limit: 1,
    depth: 0,
  ...
  const media = await payload.create({
    collection: 'media',
    data: {
      alt: 'Zahnarzt Kipp Website-Mockup',
    },
...
async function main() {
  ...
  const nextCentralBlock = buildCentralPortfolioCaseBlock({
    ...centralBlock,
    intro:
      'Eine Auswahl realer Kundenprojekte aus Webdesign, Marketing und Branding - inklusive SEO-, Lead-, Medizin-, Praxis- und E-Commerce-Referenzen mit messbaren Ergebnissen.',
    cases: updatedCases,
  ...
  console.log('\nFertig. Zahnarzt Kipp ist im zentralen Portfolio-Slider gepflegt.')
}

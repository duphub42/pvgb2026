/**
 * Fuegt ZHKplus - Zahnheilkunde Plus als zentralen Portfolio-Case hinzu.
 *
 * Ausfuehren:
 *   npx tsx src/scripts/upsert-zhkplus-portfolio-case.ts
 *   npx tsx src/scripts/upsert-zhkplus-portfolio-case.ts --dry-run
 */
...
const CASE_TITLE = 'ZHKplus - Zahnheilkunde Plus'
const MOCKUP_PATH = '/Users/horus/Downloads/ZHKplus-mockup.png'
...
async function getOrCreateMedia(dryRun: boolean): Promise<number | undefined> {
  ...
  const existing = await payload.find({
    ...
    where: {
      or: [
        { filename: { equals: 'ZHKplus-mockup.png' } },
        { alt: { equals: 'ZHKplus Zahnheilkunde Plus Portal-Mockup' } },
      ],
    },
    limit: 1,
    depth: 0,
  ...
  const media = await payload.create({
    collection: 'media',
    data: {
      alt: 'ZHKplus Zahnheilkunde Plus Portal-Mockup',
    },
...
async function main() {
  ...
  console.log('\nFertig. ZHKplus ist im zentralen Portfolio-Slider gepflegt.')
}

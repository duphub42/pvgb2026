...
export const PhilippBacherHero: React.FC<Page['hero']> = (props) => {
  ...
  const foregroundScale = 1 + Math.min(currentOffset * 0.001, 0.07)
  const dynamicOverlayOpacity = Math.min((currentOffset / 1000) + (overlayOpacity ?? 0.5), 0.8)
      ...
      <div className="container relative z-10 flex flex-col items-center text-center px-4 md:px-0 pb-16 pt-24 md:pt-32">
        ...
        {Array.isArray(links) && links.length > 0 && (
          ...
          >
            {links.map((linkItem, i) => {
              ...
              return <CMSLink key={i} {...linkItem.link} className="w-full md:w-auto" />
            })}
          </div>
        )}
        ...
        {foregroundImage && typeof foregroundImage !== 'string' && (
          <div
            ...
            style={{ transform: `translateY(${currentOffset * 0.2}px) scale(${foregroundScale})` }}
          >

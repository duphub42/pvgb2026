...
export default function Hero() {
  return (
    <section className="relative w-full min-h-screen overflow-hidden bg-black" aria-label="Hero">
      ...
      {/* HeroBox-Container: responsive Abstand nach Header, einheitliche Breite */}
      <div className="pointer-events-none absolute inset-x-0 top-[calc(var(--header-height)+10vh)] bottom-[-10vh] max-h-[700px] z-[6] px-4 sm:px-6 md:px-8 lg:px-12 xl:px-[60px] overflow-hidden hero-box-animate">
        <div className="hero-box-inner h-full w-full max-w-7xl mx-auto rounded-2xl lg:rounded-3xl border border-white/5 hero-box-frame-shadow relative overflow-hidden flex flex-col lg:flex-row">
          ...
          <div className="relative z-10 flex flex-col gap-4 sm:gap-5 md:gap-7 p-4 sm:p-6 md:p-8 min-w-0 flex-1 lg:max-w-[min(100%,28rem)] xl:max-w-[32rem] pointer-events-auto">
            ...
            <div className="mt-auto pt-6 sm:pt-8 text-white">
              <div className="text-base sm:text-lg md:text-xl font-medium">Überschrift Marquee</div>
              <div className="overflow-hidden whitespace-nowrap mt-1 sm:mt-2">
          ...
          <div className="hidden lg:block relative w-full flex-shrink-0 lg:w-80 xl:w-96 lg:min-h-0">
            <img
              src={HERO_IMAGE_SRC}
              alt="Hero"
              className="absolute right-0 top-0 h-full w-[24rem] xl:w-96 object-cover object-right-bottom -right-8 xl:-right-12 -top-8"
            />
          </div>
        </div>
      </div>

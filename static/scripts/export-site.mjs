...
async function writeSupportFiles() {
  ...
  await writeFile(
    ...
    [
      ...
      '',
      '# Long-lived caching for hashed/versioned build assets (Next.js chunks under',
      "# _next/static are content-hashed; fonts, logos and hydrated-assets scripts in",
      '# this project are filename- or query-versioned on every change), so it is safe',
      '# to mark them immutable for a year.',
      ...
      '<FilesMatch "\\.(?:css|js|mjs|woff2?|ttf|svg)$">',
